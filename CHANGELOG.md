...
- **git**: allow extending conventional commit type mapping

### Fixed

- **changelog**: exit non-zero on error and keep stdout clean
- **git**: render changelog scopes in a stable order

## [1.18.0] - 2026-09-02

### Added

- **git**: support multiple scopes in conventional commits

## [1.17.0] - 2026-06-02

### Added

- **git**: map chore commits to Changed section

### Changed

- mark `xptr.Of` as deprecated in favor of Go 1.26's `new` function
- replace `xptr.Of` usage with `new` across the codebase
- **deps**: update dependencies and bump Go version to 1.26.0

### Fixed

- correct error formatting in `xjson/format_test.go` and `xgrpc/errors.go`

## [1.16.0] - 2025-11-19

### Added

- **git**: implement core changelog generation utilities
- **cmd**:
    - changelog ignores duplicate entries
    - add support for skipping specified types and scopes in changelog generation
    - changelog supports --tag-only
    - changelog supports --tag-branch
- **xjson**: add Format function production JSON string from any

### Changed

- upgrade direct dependencies; use go1.24.0

## [1.15.2] - 2024-12-10

### Fixed

- **cmd**: support unscoped commits generating changelog

## [1.15.1] - 2024-12-08

### Fixed

- **xmaps**: Value method now returns correctly V instead of any
